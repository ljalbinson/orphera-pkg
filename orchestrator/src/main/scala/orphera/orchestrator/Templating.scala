...
object Templating:
  ...
  /** vars values are typically String, but may also be nested
    * java.util.Map[String, Any] (see PlaybookRunner.buildNestedNodeFacts) to
    * support Mustache's dotted-name traversal, e.g. {{nodes.tst0.hostname}}
    * walking nodes -> tst0 -> hostname as real nested maps rather than a
    * literal flat string key. Mustache does NOT treat a flat key containing
    * dots as a single lookup — it always tries to traverse.
    */
