...
object FileTransfer:
  ...
  def send(request: FetchFile): Stream[IO, FileData] =
    ...
    Stream
      .eval(IO.blocking(Files.exists(path)))
      .flatMap { exists =>
        if !exists then
          Stream.emit(
            FileData(
              FileData.Payload.Error(s"File does not exist: ${request.path}")
            )
          )
        else
      ...
      .handleErrorWith { err =>
        Stream.emit(
          FileData(FileData.Payload.Error(s"Read failed: ${err.getMessage}"))
        )
      }
  ...
  private def readMetadata(path: Path): IO[FileData] =
    IO.blocking {
      ...
      FileData(
        FileData.Payload.Metadata(
          FileMetadata(path.toString, owner, group, mode)
        )
      )
    }
